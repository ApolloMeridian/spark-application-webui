...
await page.locator('.ant-tabs-tabpane-active .ant-card-head-title', { hasText: '历史资源用量' }).waitFor({ state: 'visible' });
...
await page.getByRole('button', { name: '查看持久化 Executor 日志' }).first().click();
...
await page.getByRole('menu').getByText('提交 Spark 作业', { exact: true }).click();
await page.waitForURL('**/submit');
await page.getByRole('button', { name: '提交到集群' }).waitFor({ state: 'visible' });
