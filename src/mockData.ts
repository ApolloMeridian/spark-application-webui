...
function makeApp(index: number, name: string, state: ApplicationState, namespace: string, owner: string): SparkApplication {
  ...
  const execs = Array.from({ length: executorCount }, (_, i) => ({
    ...
    node: pending && i === executorCount - 1 ? undefined : `worker-${i % 2 ? 'base' : 'auto'}-${String(i + 1).padStart(2, '0')}`,
  ...
  return {
    id: `app-${index + 1}`, name, namespace, cluster: 'spark-demo', owner,
    state, createdAt: base.subtract(index * 17 + 28, 'minute').toISOString(), startedAt, finishedAt: finish,
    image: 'spark:3.5.3', sparkVersion: '3.5.3',
    ...
    driverNode: pending ? undefined : 'worker-base-01', driverNodePool: pending ? undefined : 'baseline',
    driver: {
    ...
    events: [
      { id: `${index}-1`, type: 'Normal', reason: 'Scheduled', message: `Successfully assigned ${namespace}/${driverPod} to worker-base-01`, source: 'default-scheduler', timestamp: base.subtract(22, 'minute').toISOString(), count: 1 },
    ...
    logs: [
      ...
      '2026-09-15 09:31:08 INFO FileSourceStrategy: Reading data from s3a://demo-data/input/',
...
export const seedApplications: SparkApplication[] = [
  makeApp(0, 'parquet-to-iceberg', 'RUNNING', 'spark-prod', 'demo-user'),
  makeApp(1, 'can-merge-hourly', 'COMPLETED', 'spark-prod', 'system'),
  makeApp(2, 'etl-parser', 'FAILED', 'spark-prod', 'demo-user'),
  makeApp(3, 'customer-feature-daily', 'PENDING', 'spark-ml', 'ml-user'),
  makeApp(4, 'cdc-compaction', 'SUBMITTED', 'spark-streaming', 'system'),
  makeApp(5, 'fraud-score-backfill', 'FAILING', 'spark-ml', 'ml-user'),
  makeApp(6, 'warehouse-reconcile', 'SUBMISSION_FAILED', 'spark-prod', 'data-user'),
  makeApp(7, 'ad-hoc-session-42', 'UNKNOWN', 'spark-sandbox', 'analyst-user'),
  makeApp(8, 'recommendation-training', 'RUNNING', 'spark-ml', 'ml-user'),
