        ...
        <Header className="topbar">
          ...
          <div className="topbar-actions">
            ...
            <Dropdown
              ...
              menu={{ items: [
                { key: 'label', label: <Typography.Text type="secondary">{t('role')}: {session?.role}</Typography.Text>, disabled: true },
                ...(runtimeConfig.auth.mode === 'mock' ? (['viewer', 'admin'] as UserRole[]).map((role) => ({ key: role, label: role, icon: session?.role === role ? <span className="menu-check">✓</span> : <span /> })) : []),
                ...(runtimeConfig.auth.mode === 'local' ? [{ key: 'profile', label: t('myProfile'), icon: <UserOutlined /> }] : []),
                { type: 'divider' }, { key: 'logout', label: t('signOut'), icon: <LogoutOutlined />, danger: true },
              ], onClick: ({ key }) => { setUserMenuOpen(false); if (key === 'logout') { void logout().then(() => navigate('/login')); } else if (key === 'profile') navigate('/profile'); else if (key !== 'label') setRole(key as UserRole); } }}
            >
              <Button type="text" className="user-button"><Avatar size={30} icon={<UserOutlined />} /><span className="user-copy"><b>{session?.displayName || session?.username}</b><Tag bordered={false}>{session?.role}</Tag></span></Button>
            </Dropdown>
          </div>
        </Header>
