...
import { displayNow, formatCpu, formatMemory, formatTimestamp, stateColor } from '../utils';
...
export function OverviewPage() {
  ...
  return <>
    <PageHeader title={t('overview')} subtitle={`${runtimeConfig.cluster.name} · ${lastUpdated ? formatTimestamp(lastUpdated, 'HH:mm:ss') : '—'}`} extra={<Space wrap><DatePicker.RangePicker aria-label={t('historyRange')} allowClear={false} value={historyRange} presets={[{ label: '24h', value: [displayNow().subtract(1, 'day'), displayNow()] }, { label: '7d', value: [displayNow().subtract(7, 'day'), displayNow()] }, { label: '30d', value: [displayNow().subtract(30, 'day'), displayNow()] }, { label: '90d', value: [displayNow().subtract(90, 'day'), displayNow()] }]} onChange={(value) => { if (value?.[0] && value[1]) setHistoryRange([value[0].startOf('day'), value[1].endOf('day')]); }} /><Button onClick={load}>{t('refresh')}</Button></Space>} />
