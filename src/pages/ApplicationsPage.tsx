...
export function ApplicationsPage() {
  ...
  const columns: ColumnsType<SparkApplication> = [
    { title: t('application'), dataIndex: 'name', width: 230, sorter: (a, b) => a.name.localeCompare(b.name), render: (value, row) => <div className="app-cell"><span className="spark-mini-mark">S</span><span><Typography.Link onClick={() => navigate(`/applications/${row.namespace}/${row.name}`)}>{value}</Typography.Link><small>{row.namespace}</small></span></div> },
    ...
    { title: t('createdAt'), dataIndex: 'createdAt', width: 165, sorter: (a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime(), render: (value) => formatTimestamp(value) },
    { title: t('finishedAt'), dataIndex: 'finishedAt', width: 165, sorter: (a, b) => new Date(a.finishedAt ?? 0).getTime() - new Date(b.finishedAt ?? 0).getTime(), render: (value) => value ? formatTimestamp(value) : '—' },
