import { DeleteOutlined, EyeOutlined, ReloadOutlined, SearchOutlined, StopOutlined } from '@ant-design/icons';
...
export function ApplicationsPage() {
  ...
  const [state, setState] = useState<ApplicationState | 'ALL'>(initialState || 'ALL'); const [namespace, setNamespace] = useState<string>('ALL'); const [target, setTarget] = useState<{ application: SparkApplication; operation: 'kill' | 'delete' }>();
  ...
  const filtered = useMemo(() => apps.filter((app) => (!keyword.trim() || [app.name, app.owner, app.team].some((v) => v.toLowerCase().includes(keyword.toLowerCase()))) && (state === 'ALL' || app.state === state) && (namespace === 'ALL' || app.namespace === namespace)), [apps, keyword, state, namespace]);
  const columns: ColumnsType<SparkApplication> = [
    ...
    { title: t('action'), key: 'action', width: 130, fixed: 'right', render: (_, row) => <Space><Tooltip title={t('details')}><Button size="small" type="text" icon={<EyeOutlined />} onClick={() => navigate(`/applications/${row.namespace}/${row.name}`)} /></Tooltip>{runtimeConfig.features.kill && row.state === 'RUNNING' && <Tooltip title={canKill(session?.role ?? 'viewer') ? t('kill') : t('noPermission')}><Button size="small" danger type="text" disabled={!canKill(session?.role ?? 'viewer')} icon={<StopOutlined />} onClick={() => setTarget({ application: row, operation: 'kill' })} /></Tooltip>}{runtimeConfig.features.kill && isTerminalState(row.state) && <Tooltip title={canDelete(session?.role ?? 'viewer') ? t('delete') : t('noDeletePermission')}><Button size="small" danger type="text" disabled={!canDelete(session?.role ?? 'viewer')} icon={<DeleteOutlined />} onClick={() => setTarget({ application: row, operation: 'delete' })} /></Tooltip>}</Space> },
  ];
  ...
  return <>
    ...
    <Card className="filter-card"><div className="filter-bar"><Input allowClear value={keyword} onChange={(e) => setKeyword(e.target.value)} prefix={<SearchOutlined />} placeholder={t('search')} className="search-input" /><Select value={state} onChange={setState} placeholder={t('status')} options={['ALL', 'RUNNING', 'COMPLETED', 'FAILED', 'SUBMISSION_FAILED', 'PENDING', 'SUBMITTED', 'FAILING', 'UNKNOWN', 'KILLED'].map((value) => ({ value, label: value === 'ALL' ? t('all') : value }))} /><Select value={namespace} onChange={setNamespace} placeholder={t('namespace')} options={[{ value: 'ALL', label: t('all') }, ...[...new Set(apps.map((app) => app.namespace))].map((value) => ({ value, label: value }))]} /><Button onClick={() => { setKeyword(''); setState('ALL'); setNamespace('ALL'); }}>{t('resetFilters')}</Button></div></Card>
