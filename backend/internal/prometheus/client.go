...
func (c *Client) Usage(ctx context.Context, namespace string, podNames []string) (map[string]PodUsage, []domain.MetricPoint, error) {
	...
	return c.UsageRange(ctx, namespace, podNames, end.Add(-c.lookback), end)
}

func (c *Client) UsageRange(ctx context.Context, namespace string, podNames []string, start, end time.Time) (map[string]PodUsage, []domain.MetricPoint, error) {
	...
	for index, candidates := range queries {
		...
		go func(index int, candidates []queryCandidate) {
			defer wg.Done()
			response, err := c.queryRangeWithFallback(ctx, candidates, start, end)
