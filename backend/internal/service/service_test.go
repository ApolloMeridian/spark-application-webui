...
func (f *fakeKubernetes) TerminatePodByDeadline(_ context.Context, _, name string) error {
	if f.podDeleteErr != nil {
		return f.podDeleteErr
	}
	f.driverTerminated = true
	f.deletedPods = append(f.deletedPods, "deadline:"+name)
	return nil
}
...
func (fakePrometheus) UsageRange(context.Context, string, []string, time.Time, time.Time) (map[string]metrics.PodUsage, []domain.MetricPoint, error) {
	return map[string]metrics.PodUsage{}, []domain.MetricPoint{{Time: "2026-09-20T00:01:00Z", CPU: 0.5, MemoryGiB: 1}}, nil
}
...
func (f fakeLogSource) QueryLogs(context.Context, string, string, time.Time, time.Time, string, int) ([]domain.LogEntry, error) {
...
func TestListMapsRealResources(t *testing.T) {
	...
	app := apps[0]
	if app.State != "RUNNING" || app.Owner != "unknown" || app.Driver.Current == nil || app.Driver.Current.CPU != 0.5 {
...
func TestKillRetainsApplicationAndForceDeletesPods(t *testing.T) {
	...
	audit, err := svc.KillApplication(context.Background(), "spark", "demo", "alice", "test")
	if err != nil || kubernetes.deleted || !kubernetes.restartDisabled || !kubernetes.driverTerminated || audit.Result != "SUCCESS" || len(store.rows) != 1 {
		t.Fatalf("unexpected kill result: appDeleted=%v restartDisabled=%v driverTerminated=%v audit=%#v rows=%d err=%v", kubernetes.deleted, kubernetes.restartDisabled, kubernetes.driverTerminated, audit, len(store.rows), err)
	}
	if audit.Operation != "KILL" || audit.Operator != "alice" || len(kubernetes.deletedPods) != 2 || kubernetes.deletedPods[0] != "deadline:demo-driver" || kubernetes.deletedPods[1] != "demo-exec-1" {
		t.Fatalf("expected retained failed driver and deleted executor, pods=%v audit=%#v", kubernetes.deletedPods, audit)
	}
}

func TestKillSubmittedApplicationWithPendingDriver(t *testing.T) {
	object := runningObject()
	object["status"].(map[string]any)["applicationState"] = map[string]any{"state": "SUBMITTED"}
	kubernetes := &fakeKubernetes{object: object, pods: []kube.Pod{{Name: "demo-driver", Role: "driver", Phase: "PENDING", Labels: map[string]string{"sparkoperator.k8s.io/app-name": "demo"}}}}
	store := &fakeStore{}
	svc := New(config.Config{Namespaces: []string{"spark"}}, kubernetes, fakePrometheus{}, store, slog.New(slog.NewTextHandler(io.Discard, nil)))
	audit, err := svc.KillApplication(context.Background(), "spark", "demo", "alice", "image pull backoff")
	if err != nil || audit.Result != "SUCCESS" || !kubernetes.restartDisabled || !kubernetes.driverTerminated {
		t.Fatalf("submitted application should be terminable: audit=%#v err=%v", audit, err)
	}
}
...
func TestSubmitCreatesApplicationAndAudits(t *testing.T) {
	...
	app, err := svc.SubmitApplication(context.Background(), "spark", manifest, "alice")
	if err != nil || kubernetes.created == nil || app.Name != "demo" || len(store.rows) != 1 || store.rows[0].Operation != "SUBMIT" || store.rows[0].Operator != "alice" {
		t.Fatalf("unexpected submit: app=%#v created=%#v audit=%#v err=%v", app, kubernetes.created, store.rows, err)
	}
	annotations := kube.StringMap(kubernetes.created, "metadata", "annotations")
	if annotations[consoleOwnerAnnotation] != "alice" {
		t.Fatalf("expected console owner annotation, got %#v", annotations)
	}
}
...
func TestSubmitRejectsNamespaceMismatch(t *testing.T) {
	...
	manifest := "apiVersion: sparkoperator.k8s.io/v1beta2\nkind: SparkApplication\nmetadata:\n  name: demo\n  namespace: other\n"
	if _, err := svc.SubmitApplication(context.Background(), "spark", manifest, "alice"); err == nil {
...
func TestExecutorLogsValidateApplicationOwnership(t *testing.T) {
	object := runningObject()
	object["status"].(map[string]any)["executorState"] = map[string]any{"demo-exec-1": "RUNNING"}
	svc := New(config.Config{Namespaces: []string{"spark"}}, &fakeKubernetes{object: object}, fakePrometheus{}, &fakeStore{}, slog.New(slog.NewTextHandler(io.Discard, nil)), fakeLogSource{entries: []domain.LogEntry{{Line: "ready"}}})
	entries, err := svc.GetExecutorLogs(context.Background(), "spark", "demo", "demo-exec-1", time.Now().Add(-time.Hour), time.Now(), "forward", 100)
	if err != nil || len(entries) != 1 || entries[0].Line != "ready" {
		t.Fatalf("unexpected executor logs: %#v err=%v", entries, err)
	}
	if _, err := svc.GetExecutorLogs(context.Background(), "spark", "demo", "another-pod", time.Now().Add(-time.Hour), time.Now(), "forward", 100); err == nil {
		t.Fatal("expected unrelated pod to be rejected")
	}
}
...
func TestDeleteTerminalApplicationAndAudit(t *testing.T) {
	...
	store := &fakeStore{}
	svc := New(config.Config{Namespaces: []string{"spark"}}, kubernetes, fakePrometheus{}, store, slog.New(slog.NewTextHandler(io.Discard, nil)))
	audit, err := svc.DeleteApplication(context.Background(), "spark", "demo", "alice", "cleanup")
	if err != nil || !kubernetes.deleted || audit.Operation != "DELETE" || audit.Result != "SUCCESS" || audit.Operator != "alice" {
