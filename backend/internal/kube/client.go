...
func (c *Client) TerminatePodByDeadline(ctx context.Context, namespace, name string) error {
	if err := c.ensureNamespace(namespace); err != nil {
		return err
	}
	patch := map[string]any{"spec": map[string]any{"activeDeadlineSeconds": 1}}
	_, err := c.do(ctx, http.MethodPatch, "/api/v1/namespaces/"+url.PathEscape(namespace)+"/pods/"+url.PathEscape(name), patch, "application/merge-patch+json")
	return err
}
